...
- Adds a direct PyObjC macOS backend modeled on the working `anki-xbox-controller-mac` GameController → CoreHaptics path; it is preferred automatically when Apple reports a haptic controller.
- Loads PyObjC from Anki when available, or installs the four required binary framework packages into a user-writable Anki data directory in the background without administrator access or changes to Anki.app.
